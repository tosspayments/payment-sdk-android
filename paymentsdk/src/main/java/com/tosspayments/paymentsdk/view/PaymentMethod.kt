...
class PaymentMethod(context: Context, attrs: AttributeSet? = null) :
    ...
    class PaymentInfo private constructor(
        builder: Builder
    ) : TossPaymentInfo(builder.orderId, builder.orderName, builder.amount) {

        val taxExemptionAmount: Number
        val useEscrow: Boolean?
        val escrowProducts: List<EscrowProduct>?
        val customerMobilePhone: String?
        val showCustomerMobilePhone: Boolean
        val mobileCarrier: List<TossPaymentMobileCarrier>?
        val orderId: String
